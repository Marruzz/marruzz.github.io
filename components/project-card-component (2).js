...
class ProjectCardComponent extends HTMLElement {
  connectedCallback() {
    ...
    const challenges = this.getAttribute('challenges') || '';
    const solutions = this.getAttribute('solutions') || '';
    ...
    if (liveUrl) {
      ...
      <div class="project-card bg-white rounded-3xl overflow-hidden shadow-xl hover:shadow-2xl border border-gray-100 group h-full flex flex-col">
        ...
        <div class="p-6 flex-grow flex flex-col">
          ...
          <!-- Project Purpose (if provided) -->
          ${purpose ? `
          <div class="mb-4">
            <h4 class="text-sm font-semibold text-gray-800 mb-2 flex items-center">
              <i class="fas fa-bullseye text-blue-600 mr-2"></i>Scopo del Progetto
            </h4>
            <p class="text-xs text-gray-600 leading-relaxed">${purpose}</p>
          </div>
          ` : ''}
          
          <!-- Technical Challenges & Solutions (if provided) -->
          ${challenges && solutions ? `
          <div class="mb-4">
            <h4 class="text-sm font-semibold text-gray-800 mb-2 flex items-center">
              <i class="fas fa-cogs text-orange-600 mr-2"></i>Sfide Tecniche
            </h4>
            <div class="space-y-2">
              <div class="bg-orange-50 border-l-4 border-orange-400 p-2 rounded-r">
                <p class="text-xs text-orange-800 font-medium">Sfida:</p>
                <p class="text-xs text-orange-700">${challenges}</p>
              </div>
              <div class="bg-green-50 border-l-4 border-green-400 p-2 rounded-r">
                <p class="text-xs text-green-800 font-medium">Soluzione:</p>
                <p class="text-xs text-green-700">${solutions}</p>
              </div>
            </div>
          </div>
          ` : ''}
          ...
          <div class="mb-4">
            <h4 class="text-sm font-semibold text-gray-800 mb-2 flex items-center">
              <i class="fas fa-layer-group text-purple-600 mr-2"></i>Tecnologie Utilizzate
            </h4>
